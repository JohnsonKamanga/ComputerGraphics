...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        trans = rotate(trans, radians(float(M_PI/6.0f)), vec3(0.0, 0.0, 1.0));
        trans = scale(trans, vec3(2.0f * float(sin(glfwGetTime())), 2.0f * float(sin(glfwGetTime())), float(sin(glfwGetTime()))));
        unsigned int transLocation = glGetUniformLocation(shaderProgram, "transform");
        glUniformMatrix4fv(transLocation, 1, GL_FALSE, value_ptr(trans));
